...
class ReportGenerator:
    ...
    async def generate_pdf(self, task_id: uuid.UUID) -> bytes:
        ...
        if symbolic:
            ...
            mattr = symbolic.get('mattr', 'N/A')
            coleman = symbolic.get('coleman_liau_index', 'N/A')
            smog = symbolic.get('smog_index', 'N/A')
            gunning = symbolic.get('gunning_fog', 'N/A')
            ...
            metric_data = [
                ...
                ["MATTR (Moving-Average TTR)", f"{mattr}%", "Robust Richness"],
                ...
                ["SMOG Index", str(smog), "12 - 14 (Academic)"],
                ["Gunning Fog", str(gunning), "12 - 16 (Academic)"],
                ["Coleman-Liau Grade", str(coleman), "12 - 16 (Academic)"],
                ["Intro/Conclusion Jaccard", f"{jaccard}%", "< 15% (Low Plagiarism)"],
        ...
        if journals:
            ...
            elements.append(Paragraph("<b>XGBoost Acceptance Prediction — SHAP Feature Explainability</b>", heading2_style))
            elements.append(Paragraph("The exact mathematical reasoning for the top matching journal's Acceptance Likelihood, generated by the SHAP (SHapley Additive exPlanations) TreeExplainer.", body_style))
            
            top_journal = journals[0]
            shap_data = top_journal.get("shap_breakdown", {})
            if shap_data:
                shap_table_data = [["Model Feature", "SHAP Impact (Percentage Points)"]]
                for feature, impact in shap_data.items():
                    color = "#16a34a" if float(impact) >= 0 else "#dc2626"
                    sign = "+" if float(impact) >= 0 else ""
                    shap_table_data.append([feature, Paragraph(f"<font color='{color}'><b>{sign}{impact}%</b></font>", body_style)])
                    
                t_shap = Table(shap_table_data, colWidths=[200, 200])
                ...
                elements.append(Paragraph("No SHAP explainability data available.", body_style))
        else:
