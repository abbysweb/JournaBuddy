...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Mathematical NLP Layer:} Calculates strict, rigorous text scores (Shannon Entropy, Simpson's Diversity, Coleman-Liau Index, TF-IDF).
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Redundancy/Plagiarism Check:} Uses Jaccard Similarity to compare Introduction vs. Conclusion overlap.
    \item \textbf{Information Density:} Calculates Shannon Entropy (bits per word) for true vocabulary richness.
    \item \textbf{Readability:} Utilizes the PDF-safe Coleman-Liau Index based entirely on character/word ratios.
    \item \textbf{Keyword Extraction:} Mathematically derives primary keywords using Term Frequency (TF).
\end{enumerate}
...
\subsection*{8.2 Journal Acceptance Predictor (Logistic Regression)}
The system replaces naive percentage guessing with a rigorous \textbf{Logistic Regression Sigmoid Algorithm} ($P = 1 / (1 + e^{-z})$). It dynamically predicts Acceptance Likelihood by weighting semantic compatibility (40\%), AI Methodological Rigor (30\%), Language Density (15\%), and Novelty (15\%).
...
\textbf{What did we just build?} \\
We turned JournaBuddy into a complete AI academic board. We replaced grade-school grammar checks with hardcore mathematical formulas. We added a built-in Plagiarism Checker that catches stolen sentences. We built a mathematical crystal ball (Logistic Regression) to predict journal acceptance rates. And finally, we print all of this into a stunning PDF certificate full of dynamic, beautifully drawn charts!
\end{tcolorbox}
